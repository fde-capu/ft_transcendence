...
export class ChatBoxComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.chatService.getChatRoom().subscribe(chatRoom => {
      this.chatRoom = chatRoom;
      this.imprint();
    });
    this.chatService.getInChatUsers().subscribe(inChat => {
      this.usersOnChat = inChat;
      this.imprint();
    });
    this.chatService.getOutOfChatUsers().subscribe(outChat => {
  ...
  isAdmin(user: User | undefined = this.user): boolean {
    for (const admin of this.chatRoom.admin) if (admin == user) return true;
