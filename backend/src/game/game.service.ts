...
import { StatisticsDTO } from 'src/user/entity/user.entity';
...
export class GameService {
  ...
  async getStats(intraId: string): Promise<StatisticsDTO> {
    const games = await this.getGameHistory(intraId);
  ...
  didUserWin(id: string, game: any): number {
    return (game.mode == 'PONG' &&
      ((game.p1_intraId == id && game.p1_goalsMade > game.p2_goalsMade) ||
        (game.p2_intraId == id && game.p2_goalsMade > game.p1_goalsMade))) ||
      (game.mode == 'PONG2' &&
        (((game.p1_intraId == id || game.p2_intraId == id) &&
          game.p1_goalsMade + game.p2_goalsMade >
            game.p3_goalsMade + game.p4_goalsMade) ||
      ...
      (game.mode == 'PONG4' &&
        ((game.p1_intraId == id &&
          game.p1_goalsMade > game.p2_goalsMade &&
          game.p1_goalsMade > game.p3_goalsMade &&
          game.p1_goalsMade > game.p4_goalsMade) ||
          (game.p2_intraId == id &&
            game.p2_goalsMade > game.p1_goalsMade &&
            game.p2_goalsMade > game.p3_goalsMade &&
            game.p2_goalsMade > game.p4_goalsMade) ||
          (game.p3_intraId == id &&
            game.p3_goalsMade > game.p1_goalsMade &&
            game.p3_goalsMade > game.p2_goalsMade &&
            game.p3_goalsMade > game.p4_goalsMade) ||
          (game.p4_intraId == id &&
            game.p4_goalsMade > game.p1_goalsMade &&
            game.p4_goalsMade > game.p2_goalsMade &&
            game.p4_goalsMade > game.p3_goalsMade)))
      ? 1
      : 0;
  }

  userGoalsMade(id: string, game: any): number {
    return game.p1_intraId == id
      ? game.p1_goalsMade
      : game.p2_intraId == id
      ? game.p2_goalsMade
      : game.p3_intraId == id
  ...
  async mockGameHistory() {
    ...
    mock = this.historyRepository.create({
      ...
      p4_intraId: 'mushka-d',
      p4_nameInGame:
        'Another random unexisten guy not even in the database (this is mock)',
      p4_scoreMade: 24354,
      p4_goalsMade: 24,
      p4_goalsTaken: 6,
